...
public class SimpleTMsItems {
    ...
    public static ItemStack getRandomTMItemStack(Pokemon pokemon) {
        ...
        List<MoveTutorItem> tmItemsOfType = allTMs.stream()
                .filter(item -> item.get() instanceof MoveTutorItem)
                .map(item -> (MoveTutorItem) item.get())
                .filter(tm -> moveNames.contains(tm.getMoveName()))
                .toList();
    ...
    public static ItemStack getRandomTRItemStack(Pokemon pokemon) {
        ...
        List<MoveTutorItem> trItemsOfType = allTRs.stream()
                .filter(item -> item.get() instanceof MoveTutorItem)
                .map(item -> (MoveTutorItem) item.get())
                .filter(tr -> moveNames.contains(tr.getMoveName()))
                .toList();
        ...
        // Choose a random TM from the filtered list
        MoveTutorItem randomTR = trItemsOfType.get(new Random().nextInt(trItemsOfType.size()));
        return new ItemStack(randomTR, 1);
    }
